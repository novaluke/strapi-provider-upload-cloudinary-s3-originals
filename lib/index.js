...
module.exports = {
  ...
  init: config => {
    ...
    const S3 = new AWS.S3({
      ...
      params: {
        Bucket: trimParam(config.s3_bucket),
      },
    });

    return {
      async upload(file) {
        return Promise.all([s3Upload(S3, file), cloudinaryUpload(file)]);
      },
      async delete(file) {
        return Promise.all([s3Delete(S3, file), cloudinaryDelete(file)]);
      },
    };
  },
};
